...
interface AuthContextValue {
  ...
  signup: (name: string, email: string, password: string, educationLevel: 'school' | 'college') => Promise<void>
  logout: () => void
  updateEducationLevel: (level: 'school' | 'college') => void
}
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const signup = async (name: string, email: string, _password: string, educationLevel: 'school' | 'college') => {
    await new Promise((r) => setTimeout(r, 400))
