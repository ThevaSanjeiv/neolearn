...
export function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      await signup(name, email, password, educationLevel)
      navigate('/dashboard')
    } catch (err) {
      ...
      <div className="w-full max-w-md bg-card border border-border rounded-2xl p-8 shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input className="w-full px-3 py-2 rounded-md border border-border bg-background" value={name} onChange={(e) => setName(e.target.value)} required />
          </div>
          <div>
          ...
          <div>
            ...
            <div className="flex gap-3">
              <button type="button" onClick={() => setEducationLevel('school')} className={`flex-1 px-4 py-2 rounded-md border ${educationLevel==='school'?'bg-primary text-primary-foreground border-primary':'border-border'}`}>School</button>
              <button type="button" onClick={() => setEducationLevel('college')} className={`flex-1 px-4 py-2 rounded-md border ${educationLevel==='college'?'bg-primary text-primary-foreground border-primary':'border-border'}`}>College</button>
            </div>
          </div>
